...
extension TenderloinViewController {
    ...
    func getProducts(startingIndex: Int = 0, completionHandler: GetProductsCompletionHandler = nil) {
        ...
        networkController.searchProducts(key: self.keyword ?? "Philips", minPrice: "\(minPrice)", maxPrice: "\(maxPrice)", isWholesale: isWholeSale ?? false, isOfficial: isOfficialStore ?? false, golds: numberOfGolds, startingIndex: startingIndex, items: 10) { (products: [Product]?, errorMessage: String?) in
